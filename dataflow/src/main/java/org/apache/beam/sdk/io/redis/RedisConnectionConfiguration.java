...
public abstract class RedisConnectionConfiguration implements Serializable {
    ...
    public static RedisConnectionConfiguration create() {
        return new AutoValue_RedisConnectionConfiguration.Builder().setHost(Protocol.DEFAULT_HOST)
                .setPort(Protocol.DEFAULT_PORT).setTimeout(Protocol.DEFAULT_TIMEOUT).setSsl(false)
                .build();
    }

    public static RedisConnectionConfiguration create(String host, int port) {
        return new AutoValue_RedisConnectionConfiguration.Builder().setHost(host).setPort(port)
                .setTimeout(Protocol.DEFAULT_TIMEOUT).setSsl(false).build();
    }
    ...
    public BinaryJedis connect() {
        BinaryJedis jedis = new BinaryJedis(host(), port(), timeout(), ssl());
        if (auth() != null) {
            jedis.auth(auth());
        }
        return jedis;
