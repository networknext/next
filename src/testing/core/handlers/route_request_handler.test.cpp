...
TEST(core_handlers_route_request_handler_sdk4_signed)
{
  ...
  packet.length = crypto::PACKET_HASH_LENGTH + 1 + RouteTokenV4::SIZE_OF_SIGNED * 2;
  packet.addr = from;
  ...
  CHECK(recorder.route_request_tx.num_packets == 1);
  CHECK(recorder.route_request_tx.num_bytes == packet.length - RouteTokenV4::SIZE_OF_SIGNED);
...
TEST(core_handlers_route_request_handler_unsigned)
{
  ...
  packet.length = 1 + RouteToken::SIZE_OF_ENCRYPTED * 2;
  packet.addr = from;
  ...
  CHECK(recorder.route_request_tx.num_packets == 1);
  CHECK(recorder.route_request_tx.num_bytes == packet.length - RouteToken::SIZE_OF_ENCRYPTED);
  ...
  CHECK(packet.length == prev_len - RouteToken::SIZE_OF_ENCRYPTED);
...
TEST(core_handlers_route_request_handler_signed)
{
  ...
  packet.length = crypto::PACKET_HASH_LENGTH + 1 + RouteToken::SIZE_OF_ENCRYPTED * 2;
  packet.addr = from;
