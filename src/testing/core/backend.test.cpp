...
Test(core_backend_init_valid)
{
  ...
  core::Backend backend(BackendHostname, RelayAddr, Keychain, routerInfo, manager, Base64RelayPublicKey, sessions, TrafficStats, requester);
  ...
  check(request.from(requester.Request));
...
Test(core_Backend_updateCycle_no_ack_for_40s_then_ack_then_wait)
{
  ...
  testing::MockHttpRequester requster;
...
Test(core_Backend_updateCycle_update_fails_for_max_number_of_attempts)
{
  ...
  auto fut = std::async(std::launch::async, [&] {
    std::this_thread::sleep_for(2s);
    requester.Success = false;  // set to false here to trigger failed updates
  });
...
Test(core_Backend_update_valid)
{
  ...
  // check the request
  {
    ...
    check(doc.parse(requester.Request));
