...
type ContinueRouteToken4 struct {
...
func (r *ContinueRouteToken4) Type() int {
	return RouteTypeContinue
}

func (r *ContinueRouteToken4) Encrypt(privateKey []byte) ([]byte, int, error) {
	r.privateKey = make([]byte, crypto.KeySize)
	rand.Read(r.privateKey)

	r.tokens = make([]byte, EncryptedContinueRouteTokenSize4*(len(r.Relays)+2))
...
func (r *ContinueRouteToken4) encryptToken(addr net.UDPAddr, receiverPublicKey []byte, senderPrivateKey []byte) {
	tokenstart := r.offset
	tokenend := r.offset + ContinueRouteTokenSize4
	...
	// Encode the data into the rest of the node
	var index int
	encoding.WriteUint64(r.tokens[nonceend:], &index, r.ExpireTimestamp)
	encoding.WriteUint64(r.tokens[nonceend:], &index, r.SessionID)
	encoding.WriteUint8(r.tokens[nonceend:], &index, r.SessionVersion)
...
func (r *NextRouteToken4) Type() int {
	return RouteTypeNew
}

func (r *NextRouteToken4) Encrypt(privateKey []byte) ([]byte, int, error) {
	if len(r.Relays) <= 0 {
		return nil, 0, errors.New("at least 1 relay is required")
	}
	...
	r.tokens = make([]byte, EncryptedNextRouteTokenSize4*(len(r.Relays)+2))
...
// and increments the overall offset to move across and ecrypt
// each token of the tokens slice to avoid a lot of copy ops
func (r *NextRouteToken4) encryptToken(addr *net.UDPAddr, receiverPublicKey []byte, senderPrivateKey []byte) {
	tokenstart := r.offset
	tokenend := r.offset + NextRouteTokenSize4
