...
void relay_address_data( uint32_t address_big_endian, uint8_t * output )
{
	uint32_t address_little_endian = relay::bswap( address_big_endian );
	output[0] = address_little_endian & 0xFF;
	output[1] = ( address_little_endian >> 8  ) & 0xFF;
	output[2] = ( address_little_endian >> 16 ) & 0xFF;
	output[3] = ( address_little_endian >> 24 ) & 0xFF;
}
    ...
    while ( !quit )
    {
        ...
        if ( local && ( packet_id == RELAY_LOCAL_PING_PACKET ) )
        {
            if ( packet_bytes == 1 + 1 + RELAY_ADDRESS_BYTES + 8 )
            {
                ...
                token_data.source_address = relay_htonl( internal ? relay->relay_internal_address.data.ip : relay->relay_public_address.data.ip );
                token_data.source_port = relay_htons( relay->relay_public_address.port );
                token_data.dest_address = relay_htonl( to_address.data.ip );
