...
relay_platform_socket_t * relay_platform_socket_create( relay_address_t * address, int socket_type, float timeout_seconds, int send_buffer_size, int receive_buffer_size )
{
    ...
    {
        printf( "failed to set socket receive buffer size\n" );
        relay_platform_socket_destroy( socket );
        return NULL;
    }
...
void relay_platform_socket_send_packet( relay_platform_socket_t * socket, const relay_address_t * to, const void * packet_data, int packet_bytes )
{
    assert( socket );
    assert( to );
