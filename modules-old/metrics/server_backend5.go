...
// ServerBackend5Status defines the metrics reported by the service's status endpoint
type ServerBackend5Status struct {
	// Service Information
...
type ServerInit5Metrics struct {
...
// EmptyServerInit5Metrics is used for testing when we want to pass in metrics but don't care about their value.
var EmptyServerInit5Metrics = ServerInit5Metrics{
...
type ServerUpdate5Metrics struct {
	HandlerMetrics *PacketHandlerMetrics
...
var EmptyServerUpdate5Metrics = ServerUpdate5Metrics{
	HandlerMetrics:         &EmptyPacketHandlerMetrics,
	ReadPacketFailure:      &EmptyCounter{},
	BuyerNotFound:          &EmptyCounter{},
...
type SessionUpdate5Metrics struct {
	HandlerMetrics *PacketHandlerMetrics
...
// EmptySessionUpdate5Metrics is used for testing when we want to pass in metrics but don't care about their value.
var EmptySessionUpdate5Metrics = SessionUpdate5Metrics{
	HandlerMetrics:                             &EmptyPacketHandlerMetrics,
...
// MatchDataHandler5Metrics defines the set of metrics for the match data handler in the server backend.
type MatchDataHandler5Metrics struct {
	HandlerMetrics *PacketHandlerMetrics
...
// EmptyMatchDataHandler5Metrics is used for testing when we want to pass in metrics but don't care about their value.
var EmptyMatchDataHandler5Metrics = MatchDataHandler5Metrics{
	HandlerMetrics:       &EmptyPacketHandlerMetrics,
	ReadPacketFailure:    &EmptyCounter{},
	BuyerNotFound:        &EmptyCounter{},
	BuyerNotActive:       &EmptyCounter{},
...
// ServerBackend5Metrics defines the set of metrics for the server backend 5.
type ServerBackend5Metrics struct {
	ServiceMetrics *ServiceMetrics

	ServerInitMetrics       *ServerInit5Metrics
	ServerUpdateMetrics     *ServerUpdate5Metrics
	SessionUpdateMetrics    *SessionUpdate5Metrics
	MatchDataHandlerMetrics *MatchDataHandler5Metrics
	...
	ErrorMetrics *ServerBackend5ErrorMetrics
}

// EmptyServerBackend5Metrics is used for testing when we want to pass in metrics but don't care about their value.
var EmptyServerBackend5Metrics = ServerBackend5Metrics{
	ServiceMetrics:                &EmptyServiceMetrics,
	ServerInitMetrics:             &EmptyServerInit5Metrics,
	ServerUpdateMetrics:           &EmptyServerUpdate5Metrics,
	SessionUpdateMetrics:          &EmptySessionUpdate5Metrics,
	...
	ErrorMetrics:                  &EmptyServerBackend5ErrorMetrics,
}

type ServerBackend5ErrorMetrics struct {
...
var EmptyServerBackend5ErrorMetrics = ServerBackend5ErrorMetrics{
...
// NewServerBackend5Metrics creates the metrics that the server backend will use.
func NewServerBackend5Metrics(ctx context.Context, handler Handler) (*ServerBackend5Metrics, error) {
	serviceName := "server_backend5"

	var err error
	m := &ServerBackend5Metrics{}
	...
	m.ServerInitMetrics, err = newServerInit5Metrics(ctx, handler, serviceName, "server_init", "Server Init", "server init request")
	if err != nil {
		return nil, err
	}

	m.ServerUpdateMetrics, err = newServerUpdate5Metrics(ctx, handler, serviceName, "server_update", "Server Update", "server update")
	if err != nil {
		return nil, err
	}

	m.SessionUpdateMetrics, err = newSessionUpdate5Metrics(ctx, handler, serviceName, "session_update", "Session Update", "session update request")
	if err != nil {
		return nil, err
	}

	m.MatchDataHandlerMetrics, err = newMatchDataHandler5Metrics(ctx, handler, serviceName, "match_data", "Match Data", "match data entry")
	if err != nil {
		return nil, err
	}
	...
	m.BillingMetrics.Entries2Submitted, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Billing Entries 2 Submitted",
		ServiceName: serviceName,
		ID:          "billing.entries_submitted_2",
		Unit:        "entries",
		Description: "The number of billing entries 2 the server_backend5 has submitted to be published",
	})
	if err != nil {
		return nil, err
	}
	...
	m.BillingMetrics.Entries2Queued, err = handler.NewGauge(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Billing Entries 2 Queued",
		ServiceName: serviceName,
		ID:          "billing.entries_queued_2",
		Unit:        "entries",
		Description: "The number of billing entries 2 the server_backend5 has queued waiting to be published",
	})
	if err != nil {
		return nil, err
	}
	...
	m.BillingMetrics.Entries2Flushed, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Billing Entries 2 Flushed",
		ServiceName: serviceName,
		ID:          "billing.entries_flushed_2",
		Unit:        "entries",
		Description: "The number of billing entries 2 the server_backend5 has flushed after publishing",
	})
	if err != nil {
		return nil, err
	}
	...
	m.BillingMetrics.SummaryEntries2Submitted, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Billing Summary Entries 2 Submitted",
		ServiceName: serviceName,
		ID:          "billing.summary_entries_submitted_2",
		Unit:        "entries",
		Description: "The number of billing summary entries 2 the server_backend5 has submitted to be published",
	})
	if err != nil {
		return nil, err
	}
	...
	m.BillingMetrics.ErrorMetrics.Billing2PublishFailure, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Billing 2 Publish Failure",
		ServiceName: serviceName,
		ID:          "billing.publish_failure_2",
		Unit:        "entries",
		Description: "The number of billing entries 2 the server_backend5 has failed to publish",
	})
	if err != nil {
		return nil, err
	}
	...
	m.RefreshedMagicValues, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Refreshed Magic Values Count",
		ServiceName: serviceName,
		ID:          "refreshed_magic_values.count",
		Unit:        "count",
	...
	m.ErrorMetrics, err = newServerBackend5ErrorMetrics(ctx, handler, serviceName)
	if err != nil {
		return nil, err
	}
...
func newServerBackend5ErrorMetrics(ctx context.Context, handler Handler, serviceName string) (*ServerBackend5ErrorMetrics, error) {
	var err error
	m := &ServerBackend5ErrorMetrics{}

	m.RouteMatrixReaderNil, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Route Matrix Reader Nil",
		ServiceName: serviceName,
		ID:          "server_backend5.route_matrix_reader_nil",
		Unit:        "errors",
	...
	m.RouteMatrixReadFailure, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Route Matrix Read Failure",
		ServiceName: serviceName,
		ID:          "server_backend5.route_matrix_read_failure",
		Unit:        "errors",
	...
	m.RouteMatrixBufferEmpty, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Route Matrix Buffer Empty",
		ServiceName: serviceName,
		ID:          "server_backend5.route_matrix_buffer_empty",
		Unit:        "errors",
	...
	m.RouteMatrixSerializeFailure, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Route Matrix Serialize Failure",
		ServiceName: serviceName,
		ID:          "server_backend5.route_matrix_serialize_failure",
		Unit:        "errors",
	...
	m.BinWrapperEmpty, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Bin Wrapper Empty",
		ServiceName: serviceName,
		ID:          "server_backend5.bin_wrapper_empty",
		Unit:        "errors",
	...
	m.BinWrapperFailure, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Bin Wrapper Failure",
		ServiceName: serviceName,
		ID:          "server_backend5.bin_wrapper_failure",
		Unit:        "errors",
	...
	m.StaleRouteMatrix, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Route Matrix Stale",
		ServiceName: serviceName,
		ID:          "server_backend5.route_matrix_stale",
		Unit:        "count",
	...
	m.MagicReaderNil, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Magic Reader Nil",
		ServiceName: serviceName,
		ID:          "server_backend5.magic_reader_nil",
		Unit:        "errors",
		Description: "The number of times the " + serviceName + "'s magic reader was nil after making an HTTP GET to the magic frontend LB.",
	})
	if err != nil {
		return nil, err
	}

	m.MagicReadFailure, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Magic Read Failure",
		ServiceName: serviceName,
		ID:          "server_backend5.magic_read_failure",
		Unit:        "errors",
		Description: "The number of times the " + serviceName + " failed to read the magic values.",
	})
	if err != nil {
		return nil, err
	}

	m.MagicBufferEmpty, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Magic Buffer Empty",
		ServiceName: serviceName,
		ID:          "server_backend5.magic_buffer_empty",
		Unit:        "errors",
		Description: "The number of times the " + serviceName + "'s magic reader buffer was empty.",
	})
	if err != nil {
		return nil, err
	}

	m.MagicUnexpectedLengthError, err = handler.NewCounter(ctx, &Descriptor{
		DisplayName: "Server Backend 5 Magic Unexpected Length Error",
		ServiceName: serviceName,
		ID:          "server_backend5.magic_unexpected_length_error",
		Unit:        "errors",
		Description: "The number of times the " + serviceName + " received magic values that were not 24 bytes.",
	})
	if err != nil {
		return nil, err
	}
...
func newServerInit5Metrics(ctx context.Context, handler Handler, serviceName string, handlerID string, handlerName string, packetDescription string) (*ServerInit5Metrics, error) {
	var err error
	m := &ServerInit5Metrics{}
...
func newServerUpdate5Metrics(ctx context.Context, handler Handler, serviceName string, handlerID string, handlerName string, packetDescription string) (*ServerUpdate5Metrics, error) {
	var err error
	m := &ServerUpdate5Metrics{}
...
func newSessionUpdate5Metrics(ctx context.Context, handler Handler, serviceName string, handlerID string, handlerName string, packetDescription string) (*SessionUpdate5Metrics, error) {
	var err error
	m := &SessionUpdate5Metrics{}
...
func newMatchDataHandler5Metrics(ctx context.Context, handler Handler, serviceName string, handlerID string, handlerName string, packetDescription string) (*MatchDataHandler5Metrics, error) {
	var err error
	m := &MatchDataHandler5Metrics{}
