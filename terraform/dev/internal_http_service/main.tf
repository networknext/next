...
resource "google_compute_region_backend_service" "service" {
  ...
  load_balancing_scheme = "INTERNAL_MANAGED"
  timeout_sec           = 10
  health_checks         = [google_compute_region_health_check.service-lb.id]
  backend {
...
resource "google_compute_region_health_check" "service-lb" {
...
resource "google_compute_health_check" "service-vm" {
...
resource "google_compute_region_instance_group_manager" "service" {
  ...
  auto_healing_policies {
    health_check      = google_compute_health_check.service-vm.id
