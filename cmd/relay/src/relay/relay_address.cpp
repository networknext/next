...
namespace relay
{
    ...
    bool RelayAddress::parse(const std::string& address_string)
    {
        ...
        if (address_string[0] == '[') {
            ...
            for (int i = 0; i < 6; ++i) {
                ...
                if (index < 0) {
                    ...
                    this->mPort = std::strtoul(address_string.substr(index + 1).begin().base(), nullptr, 10);
        ...
        uint32_t addr4;
        if (relay_platform_inet_pton4(without_port.c_str(), &addr4) == RELAY_OK) {
