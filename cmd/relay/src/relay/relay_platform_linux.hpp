...
#define RELAY_PLATFORM_THREAD_FUNC
    ...
    relay_platform_socket_t* relay_platform_socket_create(
        relay::RelayAddress& address, int socket_type, float timeout_seconds, int send_buffer_size, int receive_buffer_size);

    void relay_platform_socket_destroy(relay_platform_socket_t* socket);

    void relay_platform_socket_send_packet(
        relay_platform_socket_t* socket, const relay::RelayAddress& to, const void* packet_data, int packet_bytes);

    int relay_platform_socket_receive_packet(
        relay_platform_socket_t* socket, const relay::RelayAddress& from, void* packet_data, int max_packet_size);
