...
static void test_bitpacker()
{
    ...
    util::BitWriter writer(buffer, BufferSize);
    ...
    util::BitReader reader(buffer, bytesWritten);
...
{
    {
        ...
        check(relay_address_parse(&address,
...
#define CRYPTO_BOX_CIPHERTEXT_LEN (crypto_box_MACBYTES + CRYPTO_BOX_MESSAGE_LEN)
    ...
    check(crypto_box_open_easy(decrypted, ciphertext, CRYPTO_BOX_CIPHERTEXT_LEN, nonce, sender_publickey, receiver_secretkey) ==
          0);
