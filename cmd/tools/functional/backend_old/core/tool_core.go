...
type RouteSlice struct {
	FallbackFlags  uint32
	SliceFlags     uint64
	RouteSample    RouteSample
	PredictedRoute *Route
}

const RouteSliceVersion = uint32(1)
...
func (slice *RouteSlice) Serialize(stream Stream) error {
	...
	stream.SerializeUint32(&slice.FallbackFlags)
	stream.SerializeUint64(&slice.SliceFlags)
...
func (database *RelayDatabase) UpdateRelay(update *RelayUpdate, name string, datacenterName string) bool {
	id := update.Id
	...
	relayData.PingKey = update.PingKey
	relayData.LastUpdateTime = uint64(time.Now().Unix())
	relayData.Datacenter = update.Datacenter
	relayData.DatacenterName = datacenterName
	relayData.Usage = update.Usage
...
const HistorySize_RTT = 6
const HistorySize_Jitter = 6
const HistorySize_PacketLoss = 25 
...
type StatsEntryRelay struct {
	...
	jitterIndex       int
	packetLossIndex   int
	rttHistory        [HistorySize_RTT]float32
	jitterHistory     [HistorySize_Jitter]float32
	packetLossHistory [HistorySize_PacketLoss]float32
}
...
func HistoryClear(history []float32) {
	for i := 0; i < len(history); i++ {
		history[i] = InvalidHistoryValue
	}
}
...
func (database *StatsDatabase) ProcessStats(statsUpdate *RelayStatsUpdate) {
	...
	for _, stats := range statsUpdate.PingStats {
		...
		if !relayExists {
			...
			HistoryClear(relay.rttHistory[:])
			HistoryClear(relay.jitterHistory[:])
			HistoryClear(relay.packetLossHistory[:])
		}

		relay.rttHistory[relay.rttIndex] = stats.RTT
		relay.jitterHistory[relay.jitterIndex] = stats.Jitter
		relay.packetLossHistory[relay.packetLossIndex] = stats.PacketLoss

		relay.rttIndex = (relay.rttIndex + 1) % HistorySize_RTT
		relay.jitterIndex = (relay.jitterIndex + 1) % HistorySize_Jitter
		relay.packetLossIndex = (relay.packetLossIndex + 1) % HistorySize_PacketLoss
...
func AutomaticallyFillInPortNumberForRelayAddress(input string) string {
	address := &net.UDPAddr{}
	ip_string, port_string, err := net.SplitHostPort(input)
	if err != nil {
		address.IP = net.ParseIP(input)
		address.Port = 40000 // default relay port number
	} else {
...
func (database *StatsDatabase) GetCostMatrix(relays *RelayDatabase, relayConfigData map[RelayCoreID]RelayConfigData, includeJitterAndPacketLoss bool) *CostMatrix {
	...
	for i, relayData := range stableRelays {
		...
		relayConfigData, dataFound := relayConfigData[relayData.Id]
		if dataFound {
	...
	if !includeJitterAndPacketLoss {
		...
		for i := 0; i < numRelays; i++ {
			for j := 0; j < i; j++ {
				...
				if rtt != InvalidRouteValue && jitter <= MaxJitter && packetLoss <= MaxPacketLoss {
					costMatrix.RTT[ij_index] = int32(math.Floor(float64(rtt)))
				} else {
		...
		costMatrix.Jitter = make([]float32, entryCount)
		costMatrix.PacketLoss = make([]float32, entryCount)
		...
		for i := 0; i < numRelays; i++ {
			for j := 0; j < i; j++ {
				...
				ij_index := TriMatrixIndex(i, j)
				costMatrix.RTT[ij_index] = int32(math.Floor(float64(rtt)))
				costMatrix.Jitter[ij_index] = float32(jitter)
				costMatrix.PacketLoss[ij_index] = float32(packetLoss)
			}
		}
	}
...
const NEXT_FLAGS_UPGRADE_RESPONSE_TIMED_OUT = uint32(1 << 10)

const NEXT_FLAGS_COUNT = 11

const NEXT_FLAGS_COUNT_PRE_3_4_0 = 10
...
func (packet *NextBackendSessionUpdatePacket) Serialize(stream Stream, versionMajor int32, versionMinor int32, versionPatch int32) error {
	...
	if ProtocolVersionAtLeast(versionMajor, versionMinor, versionPatch, 3, 3, 4) {
		if ProtocolVersionAtLeast(versionMajor, versionMinor, versionPatch, 3, 4, 0) {
			...
			stream.SerializeBits(&packet.Flags, NEXT_FLAGS_COUNT_PRE_3_4_0)			
		}
	}
...
func WriteCostMatrix(buffer []byte, costMatrix *CostMatrix) []byte {
	...
	numDatacenters := int32(len(costMatrix.RelayDatacenters))
	binary.LittleEndian.PutUint32(buffer[index:], uint32(numDatacenters))
	index += 4

	for k, v := range costMatrix.RelayDatacenters {
...
func WriteRouteMatrix(buffer []byte, routeMatrix *RouteMatrix) []byte {
	...
	numDatacenters := int32(len(routeMatrix.RelayDatacenters))
	binary.LittleEndian.PutUint32(buffer[index:], uint32(numDatacenters))
	index += 4

	for k, v := range routeMatrix.RelayDatacenters {
...
func Optimize(costMatrix *CostMatrix, thresholdRTT int32) *RouteMatrix {
	...
	result.RelayDatacenters = costMatrix.RelayDatacenters
