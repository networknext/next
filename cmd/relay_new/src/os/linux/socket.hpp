...
namespace os
{
  ...
  {
   public:
    Socket(SocketType type, float timeout = 0.0f);
    ~Socket();

    bool create(net::Address& addr, size_t sendBuffSize, size_t recvBuffSize, bool reuse);

    // for compat only
    bool create(legacy::relay_address_t& addr, size_t sendBuffSize, size_t recvBuffSize, bool reuse);
