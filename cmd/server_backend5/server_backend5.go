    ...
    for i := 0; i < numThreads; i++ {
        go func(thread int) {
            ...
            for {
                ...
                if !core.BasicPacketFilter(data, len(packetData)) {
                    continue
                }

                {
                    to := receiveAddress // TODO: load balancer IP

                    var magic [8]byte // TODO: actual magic values (upcoming, current, previous)
                    ...
                    // TODO: advanced check on all magic types
                    if !core.AdvancedPacketFilter(packetData, magic[:], fromAddressData, fromAddressPort, toAddressData, toAddressPort, len(packetData)) {
