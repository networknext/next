...
func mainReturnWithCode() int {
    ...
    disableAdvancedPacketFilter := envvar.GetBool("SERVER_BACKEND_DISABLE_ADVANCED_PACKET_FILTER", false)
    ...
    backendLoadBalancerIP := envvar.GetAddress("SERVER_BACKEND_IP", nil)
    ...
    // Sync mmdb
    {
        maxmindSyncInterval := envvar.GetDuration("MAXMIND_SYNC_DB_INTERVAL", time.Minute*1)
    ...
    // Sync magic values
    {
        magicURI := envvar.GetString("MAGIC_URI", "")
        if magicURI == "" {
