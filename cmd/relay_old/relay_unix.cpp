...
static int fd_urandom;

// random

void next_random_bytes( uint8_t * buf, int bytes )
{
    ssize_t result = read( fd_urandom, buf, bytes );
    next_assert( result == bytes );
    (void) result;
}

// threads

int next_thread_create( next_thread_t * thread, next_thread_return_t (*fn)(void *), void * arg )
{
    return pthread_create( &thread->handle, NULL, fn, arg ) == 0 ? NEXT_OK : NEXT_ERROR;
}
...
int next_mutex_init( next_mutex_t * mutex )
{
...
// time
    ...
    int64_t next_platform_time()
    {
        return int64_t( mach_absolute_time() * timebase_info.numer / timebase_info.denom );
    }
    ...
    int64_t next_platform_time()
    {
        ...
        return int64_t( ( uint64_t( ts.tv_sec ) * 1000000000ULL ) + uint64_t( ts.tv_nsec ) );
    }
...
int next_address_resolve( const char * in, next_address_t * out )
{
    addrinfo hints;
    ...
    if ( getaddrinfo( in, "40000", &hints, &result ) == 0 )
    {
        if ( result )
        {
            if ( result->ai_addr->sa_family == AF_INET6 )
            {
                ...
                for ( int i = 0; i < 8; ++i )
                {
                    out->data.ipv6[i] = next_ntohs( ( (uint16_t*) &addr_ipv6->sin6_addr ) [i] );
                }
                out->port = next_ntohs( addr_ipv6->sin6_port );
                freeaddrinfo( result );
                return NEXT_OK;
            }
            else if ( result->ai_addr->sa_family == AF_INET )
            {
                ...
                out->port = next_ntohs( addr_ipv4->sin_port );
                freeaddrinfo( result );
                return NEXT_OK;
            }
            else
...
int next_inet_pton4( const char * address_string, uint32_t * address_out )
{
...
int next_inet_pton6( const char * address_string, uint16_t * address_out )
{
...
int next_inet_ntop6( const uint16_t * address, char * address_string, size_t address_string_size )
{
