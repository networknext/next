...
AuthHandler = {
  ...
  resendVerificationEmail() {
    ...
    JSONRPCClient
      ...
      .then((response) => {
        Object.assign(rootComponent.$data.alerts.verifyEmail, {
          show: false
        })
        Object.assign(rootComponent.$data.alerts.emailSent, {
          show: true
        })
      })
      .catch((error) => {
        ...
        Object.assign(rootComponent.$data.alerts.emailFailed, {
          show: true
        })
      })
  }
...
MapHandler = {
  ...
  updateFilter(id) {
    ...
    Object.assign(rootComponent.$data.pages.map.filter, {
      buyerId: id
    });
  ...
  refreshMapCount() {
    ...
    JSONRPCClient
      .call('BuyersService.TotalSessions', {
  ...
  async fetchCurrentUserInfo() {
    return AuthHandler.auth0Client.getIdTokenClaims()
      .then((response) => {
        ...
        return JSONRPCClient.call("AuthService.UserAccount", {
      ...
      .then((response) => {
        ...
        if (AuthHandler.isSignupRedirect && !UserHandler.isAnonymous() && !UserHandler.isAnonymousPlus() && (!UserHandler.isOwner() || !UserHandler.isAdmin())) {
          JSONRPCClient
            .call("AuthService.UpgradeAccount", {
              user_id: UserHandler.userInfo.userId
            })
            .then((response) => {
...
WorkspaceHandler = {
  ...
  changePage(page, options) {
    ...
    Object.keys(rootComponent.$data.pages).forEach((page) => {
      Object.assign(rootComponent.$data.pages[page], {
        show: false
      });
    });
  ...
  saveUser(accountInfo, index) {
    if (accountInfo.edit) {
      ...
      JSONRPCClient
        .call('AuthService.UpdateUserRoles', {
          user_id: `auth0|${accountInfo.user_id}`,
          roles: roles
        })
        .then((response) => {
    ...
    if (accountInfo.delete) {
      JSONRPCClient
        .call('AuthService.DeleteUserAccount', {
          user_id: `auth0|${accountInfo.user_id}`
        })
        .then((response) => {
  ...
  loadSettingsPage() {
    ...
    JSONRPCClient
      .call('BuyersService.GameConfiguration', {
        domain: UserHandler.userInfo.domain
      })
      .then((response) => {
  ...
  fetchSessionInfo() {
    ...
    JSONRPCClient
      .call("BuyersService.SessionDetails", {
        session_id: id
      })
      .then((response) => {
        ...
        setTimeout(() => {
          ...
          if (!this.sessionToolLoop) {
            ...
            if (this.sessionToolMapInstance) {
              this.sessionToolMapInstance.setProps({
                layers: []
              })
              this.sessionToolMapInstance.setProps({
  ...
  fetchUserSessions() {
    ...
    JSONRPCClient
      .call("BuyersService.UserSessions", {
        user_hash: hash
      })
      .then((response) => {
      ...
      if (UserHandler.isAdmin() || isDev) {
        fetch("/version", {
          ...
          response.json().then((json) => {
            ...
            Object.assign(rootComponent.$data, {
              portalVersion: `Git Hash: ${json.sha} - Commit: ${json.commit_message || "none"}`
            });
          });
        })
      }
...
function updatePubKey() {
  ...
  JSONRPCClient
    .call("BuyersService.UpdateGameConfiguration", {
      name: company,
      domain: domain,
...
function addUsers() {
  ...
  JSONRPCClient
    .call("AuthService.AddUserAccount", {
      emails: emails,
      roles: roles
    })
    .then((response) => {
      ...
      Object.assign(rootComponent.$data.pages.settings, {
        accounts: rootComponent.$data.pages.settings.accounts.concat(newAccounts)
      });
      setTimeout(() => {
