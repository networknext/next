...
func TestTopSessions(t *testing.T) {
	...
	redisServer, _ := miniredis.Run()
	redisPool := storage.NewRedisPool(redisServer.Addr(), 1, 1)
	client, err := storage.NewRawRedisClient(redisServer.Addr())
	assert.NoError(t, err)
	...
	redisServer.HSet(fmt.Sprintf("session-%s-meta", sessionID1), transport.SessionMeta{ID: 111, DeltaRTT: 50}.RedisString(), time.Hour.String())
	redisServer.HSet(fmt.Sprintf("session-%s-meta", sessionID2), transport.SessionMeta{ID: 222, DeltaRTT: 100}.RedisString(), time.Hour.String())
	redisServer.HSet(fmt.Sprintf("session-%s-meta", sessionID3), transport.SessionMeta{ID: 333, DeltaRTT: 150}.RedisString(), time.Hour.String())

	client.Command("SET", "sm-%s \"%s\" EX %d", sessionID1, transport.SessionMeta{ID: 111, DeltaRTT: 50}.RedisString(), time.Hour)
	client.Command("SET", "sm-%s \"%s\" EX %d", sessionID2, transport.SessionMeta{ID: 222, DeltaRTT: 100}.RedisString(), time.Hour)
	client.Command("SET", "sm-%s \"%s\" EX %d", sessionID3, transport.SessionMeta{ID: 333, DeltaRTT: 150}.RedisString(), time.Hour)
	...
	/* 	t.Run("filtered - !admin - sameBuyer", func(t *testing.T) {
	   		var reply jsonrpc.TopSessionsReply
	   		err := svc.TopSessions(req, &jsonrpc.TopSessionsArgs{CompanyCode: buyerID1}, &reply)
	   		assert.NoError(t, err)
