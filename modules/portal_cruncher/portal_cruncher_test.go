...
func TestNewPortalCruncher(t *testing.T) {
	...
	t.Run("top sessions failure", func(t *testing.T) {
		portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, nil, "", redisSessionMap.Addr(), redisSessionMeta.Addr(), redisSessionSlices.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 0, logger, metrics.EmptyPortalCruncherMetrics)
		assert.Nil(t, portalCruncher)
		assert.Error(t, err)
	})

	t.Run("session map failure", func(t *testing.T) {
		portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, nil, redisTopSessions.Addr(), "", redisSessionMeta.Addr(), redisSessionSlices.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 0, logger, metrics.EmptyPortalCruncherMetrics)
		assert.Nil(t, portalCruncher)
		assert.Error(t, err)
	})

	t.Run("session meta failure", func(t *testing.T) {
		portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, nil, redisTopSessions.Addr(), redisSessionMap.Addr(), "", redisSessionSlices.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 0, logger, metrics.EmptyPortalCruncherMetrics)
		assert.Nil(t, portalCruncher)
		assert.Error(t, err)
	})

	t.Run("session slices failure", func(t *testing.T) {
		portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, nil, redisTopSessions.Addr(), redisSessionMap.Addr(), redisSessionMeta.Addr(), "", useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 0, logger, metrics.EmptyPortalCruncherMetrics)
		assert.Nil(t, portalCruncher)
		assert.Error(t, err)
	})

	t.Run("bigtable emulator failure", func(t *testing.T) {
		// Unset the emulator host env var
		btEmulatorHost := os.Getenv("BIGTABLE_EMULATOR_HOST")
		err := os.Unsetenv("BIGTABLE_EMULATOR_HOST")
		assert.NoError(t, err)

		portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, nil, redisTopSessions.Addr(), redisSessionMap.Addr(), redisSessionMeta.Addr(), redisSessionSlices.Addr(), true, "", btInstanceID, btTableName, btCfName, 0, logger, metrics.EmptyPortalCruncherMetrics)
		assert.Nil(t, portalCruncher)
		assert.Error(t, err)
	...
	if useBigtable {
		t.Run("bigtable table exist failure", func(t *testing.T) {
			portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, nil, redisTopSessions.Addr(), redisSessionMap.Addr(), redisSessionMeta.Addr(), redisSessionSlices.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 0, logger, metrics.EmptyPortalCruncherMetrics)
			assert.Nil(t, portalCruncher)
			errStr := fmt.Sprintf("SetupBigtable() Could not find table %s in bigtable instance. Create the table before starting the portal cruncher", btTableName)
...
func TestReceiveMessage(t *testing.T) {
	...
	t.Run("receive error", func(t *testing.T) {
		...
		portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, subscriber, redisTopSessions.Addr(), redisSessionMap.Addr(), redisSessionMeta.Addr(), redisSessionSlices.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 0, logger, metrics.EmptyPortalCruncherMetrics)
		assert.NoError(t, err)
	...
	t.Run("count data success", func(t *testing.T) {
		...
		portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, subscriber, redisTopSessions.Addr(), redisSessionMap.Addr(), redisSessionMeta.Addr(), redisSessionSlices.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 1, logger, metrics.EmptyPortalCruncherMetrics)
		assert.NoError(t, err)
...
func TestDirectSession(t *testing.T) {
	...
	portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, subscriber, mockRedises[0].db.Addr(), mockRedises[1].db.Addr(), mockRedises[2].db.Addr(), mockRedises[3].db.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 1, logger, metrics.EmptyPortalCruncherMetrics)
	err = portalCruncher.PingRedis()
	assert.NoError(t, err)
...
func TestNoReinsertion(t *testing.T) {
	...
	portalCruncher, err := portalcruncher.NewPortalCruncher(ctx, subscriber, mockRedises[0].db.Addr(), mockRedises[1].db.Addr(), mockRedises[2].db.Addr(), mockRedises[3].db.Addr(), useBigtable, gcpProjectID, btInstanceID, btTableName, btCfName, 4, logger, metrics.EmptyPortalCruncherMetrics)
	err = portalCruncher.PingRedis()
	assert.NoError(t, err)
