...
const (
	SessionSummaryMessageVersion_Min   = 1
	SessionSummaryMessageVersion_Max   = 1
	SessionSummaryMessageVersion_Write = 1
)
...
func (message *SessionSummaryMessage) Write(buffer []byte) []byte {

	index := 0

	if message.Version < SessionSummaryMessageVersion_Min || message.Version > SessionSummaryMessageVersion_Max {
		panic(fmt.Sprintf("invalid session summary message version %d", message.Version))
	}
...
func (message *SessionSummaryMessage) Read(buffer []byte) error {

	index := 0

	if !encoding.ReadUint8(buffer, &index, &message.Version) {
		return fmt.Errorf("failed to read session summary message version")
	}

	if message.Version < SessionSummaryMessageVersion_Min || message.Version > SessionSummaryMessageVersion_Max {
		return fmt.Errorf("invalid session summary message version %d", message.Version)
	}
