...
const (
	BillingEntryVersion = uint32(9)

	MaxBillingEntryBytes = 4096
...
func (entry *BillingEntry) Serialize(stream encoding.Stream) error {
...
func (entry *BillingEntry) ClampEntry() {
	...
	if entry.DirectMinRTT < 0 {
		core.Error("BillingEntry DirectMinRTT (%d) < 0. Clamping to 0.", entry.DirectMinRTT)
		entry.DirectMinRTT = 0
	}

	if entry.DirectMinRTT > 1023 {
		core.Debug("BillingEntry DirectMinRTT (%d) > 1023. Clamping to 1023.", entry.DirectMinRTT)
		entry.DirectMinRTT = 1023
	}

	if entry.DirectMaxRTT < 0 {
		core.Error("BillingEntry DirectMaxRTT (%d) < 0. Clamping to 0.", entry.DirectMaxRTT)
		entry.DirectMaxRTT = 0
	}

	if entry.DirectMaxRTT > 1023 {
		core.Debug("BillingEntry DirectMaxRTT (%d) > 1023. Clamping to 1023.", entry.DirectMaxRTT)
		entry.DirectMaxRTT = 1023
	}

	if entry.DirectPrimeRTT < 0 {
		core.Error("BillingEntry DirectPrimeRTT (%d) < 0. Clamping to 0.", entry.DirectPrimeRTT)
		entry.DirectPrimeRTT = 0
	}

	if entry.DirectPrimeRTT > 1023 {
		core.Debug("BillingEntry DirectPrimeRTT (%d) > 1023. Clamping to 1023.", entry.DirectPrimeRTT)
		entry.DirectPrimeRTT = 1023
	}

	if entry.DirectJitter < 0 {
		core.Error("BillingEntry DirectJitter (%d) < 0. Clamping to 0.", entry.DirectJitter)
		entry.DirectJitter = 0
	}

	if entry.DirectJitter > 255 {
		core.Debug("BillingEntry DirectJitter (%d) > 255. Clamping to 255.", entry.DirectJitter)
		entry.DirectJitter = 255
	}

	if entry.DirectPacketLoss < 0 {
		core.Error("BillingEntry DirectPacketLoss (%d) < 0. Clamping to 0.", entry.DirectPacketLoss)
		entry.DirectPacketLoss = 0
	}

	if entry.DirectPacketLoss > 100 {
		core.Debug("BillingEntry DirectPacketLoss (%d) > 100. Clamping to 100.", entry.DirectPacketLoss)
		entry.DirectPacketLoss = 100
	}

	if entry.RealPacketLoss < 0 {
		core.Error("BillingEntry RealPacketLoss (%d) < 0. Clamping to 0.", entry.RealPacketLoss)
		entry.RealPacketLoss = 0
	}

	if entry.RealPacketLoss > 100 {
		core.Debug("BillingEntry RealPacketLoss (%d) > 100. Clamping to 100.", entry.RealPacketLoss)
		entry.RealPacketLoss = 100
	}

	if entry.RealJitter > 1000 {
		core.Debug("BillingEntry RealJitter (%d) > 1000. Clamping to 1000.", entry.RealJitter)
		entry.RealJitter = uint32(1000)
	}

	if len(entry.Debug) >= BillingEntryMaxDebugLength {
		core.Debug("BillingEntry Debug length (%d) >= BillingEntryMaxDebugLength (%d). Clamping to BillingEntryMaxDebugLength - 1 (%d)", len(entry.Debug), BillingEntryMaxDebugLength, BillingEntryMaxDebugLength-1)
		entry.Debug = entry.Debug[:BillingEntryMaxDebugLength-1]
	}

	if entry.RouteDiversity < 0 {
		core.Error("BillingEntry RouteDiversity (%d) < 0. Clamping to 0.", entry.RouteDiversity)
		entry.RouteDiversity = 0
	}

	if entry.RouteDiversity > 32 {
		core.Debug("BillingEntry RouteDiversity (%d) > 32. Clamping to 32.", entry.RouteDiversity)
	...
	if entry.SliceNumber == 0 || entry.Summary {
		...
		if len(entry.ClientAddress) >= BillingEntryMaxAddressLength {
			core.Debug("BillingEntry Client IP Address length (%d) >= BillingEntryMaxAddressLength (%d). Clamping to BillingEntryMaxAddressLength - 1 (%d)", len(entry.ClientAddress), BillingEntryMaxAddressLength, BillingEntryMaxAddressLength-1)
			entry.ClientAddress = entry.ClientAddress[:BillingEntryMaxAddressLength-1]
		}

		if len(entry.ISP) >= BillingEntryMaxISPLength {
			core.Debug("BillingEntry ISP length (%d) >= BillingEntryMaxISPLength (%d). Clamping to BillingEntryMaxISPLength - 1 (%d)", len(entry.ISP), BillingEntryMaxISPLength, BillingEntryMaxISPLength-1)
			entry.ISP = entry.ISP[:BillingEntryMaxISPLength-1]
		}

		if entry.ConnectionType < 0 {
			core.Error("BillingEntry ConnectionType (%d) < 0. Clamping to 0.", entry.ConnectionType)
			entry.ConnectionType = 0
		}

		// IMPORTANT: You must update this check if you ever add a new connection type in the SDK
		if entry.ConnectionType > 3 {
			core.Debug("BillingEntry ConnectionType (%d) > 3. Clamping to 0 (unknown).", entry.ConnectionType)
			entry.ConnectionType = 0
		}

		if entry.PlatformType < 0 {
			core.Error("BillingEntry PlatformType (%d) < 0. Clamping to 0.", entry.PlatformType)
			entry.PlatformType = 0
		}

		// IMPORTANT: You must update this check when you add new platforms to the SDK
		if entry.PlatformType > 10 {
			core.Debug("BillingEntry PlatformType (%d) > 10. Clamping to 0 (unknown).", entry.PlatformType)
			entry.PlatformType = 0
		}

		if len(entry.SDKVersion) >= BillingEntryMaxSDKVersionLength {
			core.Debug("BillingEntry SDKVersion length (%d) >= BillingEntryMaxSDKVersionLength (%d). Clamping to BillingEntryMaxSDKVersionLength - 1 (%d)", len(entry.SDKVersion), BillingEntryMaxSDKVersionLength, BillingEntryMaxSDKVersionLength-1)
			entry.SDKVersion = entry.SDKVersion[:BillingEntryMaxSDKVersionLength-1]
		}

		if entry.NumTags < 0 {
			core.Error("BillingEntry NumTags (%d) < 0. Clamping to 0.", entry.NumTags)
			entry.NumTags = 0
		}

		if entry.NumTags > BillingEntryMaxTags {
			core.Debug("BillingEntry NumTags (%d) > BillingEntryMaxTags (%d). Clamping to BillingEntryMaxTags (%d).", entry.NumTags, BillingEntryMaxTags, BillingEntryMaxTags)
			entry.NumTags = BillingEntryMaxTags
		}

		if len(entry.ServerAddress) >= BillingEntryMaxAddressLength {
			core.Debug("BillingEntry Server IP Address length (%d) >= BillingEntryMaxAddressLength (%d). Clamping to BillingEntryMaxAddressLength - 1 (%d)", len(entry.ServerAddress), BillingEntryMaxAddressLength, BillingEntryMaxAddressLength-1)
	...
	if entry.Summary {
		...
		if entry.NumNearRelays < 0 {
			core.Error("BillingEntry NumNearRelays (%d) < 0. Clamping to 0.", entry.NumNearRelays)
			entry.NumNearRelays = 0
		}

		if entry.NumNearRelays > BillingEntryMaxNearRelays {
			core.Debug("BillingEntry NumNearRelays (%d) > BillingEntryMaxNearRelays (%d). Clamping to BillingEntryMaxNearRelays (%d).", entry.NumNearRelays, BillingEntryMaxNearRelays, BillingEntryMaxNearRelays)
			entry.NumNearRelays = BillingEntryMaxNearRelays
		}

		for i := 0; i < int(entry.NumNearRelays); i++ {
			if entry.NearRelayRTTs[i] < 0 {
				core.Error("BillingEntry NearRelayRTT[%d] (%d) < 0. Clamping to 0.", i, entry.NearRelayRTTs[i])
				entry.NearRelayRTTs[i] = 0
			}

			if entry.NearRelayRTTs[i] > 255 {
				core.Debug("BillingEntry NearRelayRTTs[%d] (%d) > 255. Clamping to 255.", i, entry.NearRelayRTTs[i])
				entry.NearRelayRTTs[i] = 255
			}

			if entry.NearRelayJitters[i] < 0 {
				core.Error("BillingEntry NearRelayRTT[%d] (%d) < 0. Clamping to 0.", i, entry.NearRelayJitters[i])
				entry.NearRelayJitters[i] = 0
			}

			if entry.NearRelayJitters[i] > 255 {
				core.Debug("BillingEntry NearRelayJitters[%d] (%d) > 255. Clamping to 255.", i, entry.NearRelayJitters[i])
				entry.NearRelayJitters[i] = 255
			}

			if entry.NearRelayPacketLosses[i] < 0 {
				core.Error("BillingEntry NearRelayRTT[%d] (%d) < 0. Clamping to 0.", i, entry.NearRelayPacketLosses[i])
				entry.NearRelayPacketLosses[i] = 0
			}

			if entry.NearRelayPacketLosses[i] > 100 {
				core.Debug("BillingEntry NearRelayPacketLosses[%d] (%d) > 100. Clamping to 100.", i, entry.NearRelayPacketLosses[i])
	...
	if entry.Next {
		...
		if entry.NextRTT < 0 {
			core.Error("BillingEntry NextRTT (%d) < 0. Clamping to 0.", entry.NextRTT)
			entry.NextRTT = 0
		}

		if entry.NextRTT > 255 {
			core.Debug("BillingEntry NextRTT (%d) > 255. Clamping to 255.", entry.NextRTT)
			entry.NextRTT = 255
		}

		if entry.NextJitter < 0 {
			core.Error("BillingEntry NextJitter (%d) < 0. Clamping to 0.", entry.NextJitter)
			entry.NextJitter = 0
		}

		if entry.NextJitter > 255 {
			core.Debug("BillingEntry NextJitter (%d) > 255. Clamping to 255.", entry.NextJitter)
			entry.NextJitter = 255
		}

		if entry.NextPacketLoss < 0 {
			core.Error("BillingEntry NextPacketLoss (%d) < 0. Clamping to 0.", entry.NextPacketLoss)
			entry.NextPacketLoss = 0
		}

		if entry.NextPacketLoss > 100 {
			core.Debug("BillingEntry NextPacketLoss (%d) > 100. Clamping to 100.", entry.NextPacketLoss)
			entry.NextPacketLoss = 100
		}

		if entry.PredictedNextRTT < 0 {
			core.Error("BillingEntry PredictedNextRTT (%d) < 0. Clamping to 0.", entry.PredictedNextRTT)
			entry.PredictedNextRTT = 0
		}

		if entry.PredictedNextRTT > 255 {
			core.Debug("BillingEntry PredictedNextRTT (%d) > 255. Clamping to 255.", entry.PredictedNextRTT)
			entry.PredictedNextRTT = 255
		}

		if entry.NearRelayRTT < 0 {
			core.Error("BillingEntry NearRelayRTT (%d) < 0. Clamping to 0.", entry.NearRelayRTT)
			entry.NearRelayRTT = 0
		}

		if entry.NearRelayRTT > 255 {
			core.Debug("BillingEntry NearRelayRTT (%d) > 255. Clamping to 255.", entry.NearRelayRTT)
			entry.NearRelayRTT = 255
		}

		if entry.NumNextRelays < 0 {
			core.Error("BillingEntry NumNextRelays (%d) < 0. Clamping to 0.", entry.NumNextRelays)
			entry.NumNextRelays = 0
		}

		if entry.NumNextRelays > BillingEntryMaxRelays {
			core.Debug("BillingEntry NumNextRelays (%d) > BillingEntryMaxRelays (%d). Clamping to BillingEntryMaxRelays (%d).", entry.NumNextRelays, BillingEntryMaxRelays, BillingEntryMaxRelays)
