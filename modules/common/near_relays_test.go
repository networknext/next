...
func TestGetNearRelays_OutOfWay(t *testing.T) {
	...
	for i := range relayIds {
		...
		relayAddresses[i] = *core.ParseAddress(fmt.Sprintf("127.0.0.1:%d", 2000+i))
		if i < 50 {
			relayLatitudes[i] = PlayerLatitude + float64(common.RandomInt(0,+10))
			relayLongitudes[i] = PlayerLongitude + float64(common.RandomInt(0,+10))
		} else if i < 100 {
			relayLatitudes[i] = DestinationLatitude + float64(common.RandomInt(-10,0))
			relayLongitudes[i] = DestinationLongitude + float64(common.RandomInt(-10,0))
		} else {
			// out of the way
