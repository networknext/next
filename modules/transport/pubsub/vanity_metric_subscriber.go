...
// VanityMetricSubscriber is not thread safe.
type VanityMetricSubscriber struct {
	socket *zmq4.Socket

	topics []Topic
}

func NewVanityMetricSubscriber(port string, receiveBufferSize int) (*VanityMetricSubscriber, error) {
	...
	return &VanityMetricSubscriber{
		socket: socket,
	}, nil
}

func (sub *VanityMetricSubscriber) Subscribe(topic Topic) error {
	sub.topics = append(sub.topics, topic)
	return sub.socket.SetSubscribe(string(topic))
}

func (sub *VanityMetricSubscriber) Unsubscribe(topic Topic) error {
...
func (sub *VanityMetricSubscriber) Poll(timeout time.Duration) error {
...
func (sub *VanityMetricSubscriber) ReceiveMessage() <-chan MessageInfo {
...
func (sub *VanityMetricSubscriber) containsTopic(topic Topic) (bool, int) {
