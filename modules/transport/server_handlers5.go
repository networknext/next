...
func writeServerInitResponseSDK5(w io.Writer, packet *ServerInitRequestPacketSDK5, response uint32, getMagicValues func() ([8]byte, [8]byte, [8]byte), from *net.UDPAddr, to *net.UDPAddr, backendPrivateKey []byte) error {
	responsePacket := ServerInitResponsePacketSDK5{
	...
	if !core.BasicPacketFilter(responsePacketData, len(responsePacketData)) {
	...
	if _, err := w.Write(responsePacketData); err != nil {
		return err
	}
...
func writeServerResponseSDK5(w io.Writer, packet *ServerUpdatePacketSDK5, getMagicValues func() ([8]byte, [8]byte, [8]byte), from *net.UDPAddr, to *net.UDPAddr, backendPrivateKey []byte) error {
	responsePacket := ServerResponsePacketSDK5{
		RequestID: packet.RequestID,
	}
	responsePacket.UpcomingMagic, responsePacket.CurrentMagic, responsePacket.PreviousMagic = getMagicValues()
...
func writeMatchDataResponseSDK5(w io.Writer, packet *MatchDataRequestPacket, response uint32, getMagicValues func() ([8]byte, [8]byte, [8]byte), from *net.UDPAddr, to *net.UDPAddr, backendPrivateKey []byte) error {
...
func ServerInitHandlerSDK5Func(
	getDatabase func() *routing.DatabaseBinWrapper,
	getMagicValues func() ([8]byte, [8]byte, [8]byte),
	ServerTracker *storage.ServerTracker,
	backendLoadBalancerIP *net.UDPAddr,
	backendPrivateKey []byte,
	metrics *metrics.ServerInit5Metrics,
) UDPHandlerFunc {
	...
	return func(w io.Writer, incoming *UDPPacket) {
		...
		defer func() {
			if err := writeServerInitResponseSDK5(w, &packet, uint32(responseType), getMagicValues, backendLoadBalancerIP, &incoming.From, backendPrivateKey); err != nil {
		...
		datacenter, exists := database.DatacenterMap[packet.DatacenterID]
		if !exists {
			ServerTracker.AddServer(packet.BuyerID, packet.DatacenterID, incoming.From, "unknown_init")
		} else {
			ServerTracker.AddServer(packet.BuyerID, packet.DatacenterID, incoming.From, datacenter.Name)
		}
		...
		*/
		...
		if !exists {
			// core.Error("unknown datacenter [%016x, %s] for buyer id %016x", packet.DatacenterID, incoming.From.String(), packet.BuyerID)
			metrics.DatacenterNotFound.Add(1)
			return
		} else {
			core.Debug("server is in datacenter \"%s\" [%x]", datacenter.Name, packet.DatacenterID)
		}
...
func ServerUpdateHandlerSDK5Func(
	getDatabase func() *routing.DatabaseBinWrapper,
	getMagicValues func() ([8]byte, [8]byte, [8]byte),
	...
	backendLoadBalancerIP *net.UDPAddr,
	backendPrivateKey []byte,
	metrics *metrics.ServerUpdate5Metrics,
) UDPHandlerFunc {
	...
	return func(w io.Writer, incoming *UDPPacket) {
		...
		defer func() {
			if err := writeServerResponseSDK5(w, &packet, getMagicValues, backendLoadBalancerIP, &incoming.From, backendPrivateKey); err != nil {
				core.Debug("failed to write server response: %s", err)
				metrics.WriteResponseFailure.Add(1)
			}
		}()
...
func BuildNextTokensSDK5(
	sessionData *SessionDataSDK5,
	database *routing.DatabaseBinWrapper,
	buyer *routing.Buyer,
	packet *SessionUpdatePacketSDK5,
	routeNumRelays int32,
	routeRelays []int32,
	allRelayIDs []uint64,
	routerPrivateKey [crypto.KeySize]byte,
	response *SessionResponsePacketSDK5,
) {
	/*
...
type SessionHandlerStateSDK5 struct {
	...
	*/
	...
	Input SessionDataSDK5 // sent up from the SDK. previous slice.

	Output SessionDataSDK5 // sent down to the SDK. current slice.

	Writer                io.Writer
	Packet                SessionUpdatePacketSDK5
	Response              SessionResponsePacketSDK5
	PacketData            []byte
	Metrics               *metrics.SessionUpdate5Metrics
	Database              *routing.DatabaseBinWrapper
...
func SessionPreSDK5(state *SessionHandlerStateSDK5) bool {
	...
	if state.Packet.ClientPingTimedOut {
		...
		if state.PostSessionHandler.featureBilling2 {
			...
			err := UnmarshalSessionDataSDK5(&state.Input, state.Packet.SessionData[:])
	...
	if state.Packet.SliceNumber == 0 {
		...
		// Always assign location no matter the outcome of SessionPreSDK5() on the first slice
		defer func() {
...
func SessionUpdateNewSessionSDK5(state *SessionHandlerStateSDK5) {
...
func SessionUpdateExistingSessionSDK5(state *SessionHandlerStateSDK5) {
...
func SessionHandleFallbackToDirectSDK5(state *SessionHandlerStateSDK5) bool {
	...
	*/
	...
	if state.Packet.FallbackToDirect && !state.Output.FellBackToDirect {
		...
		// TODO: add the fallback to direct flags back when SDK5 supports them
...
func SessionUpdateNearRelayStatsSDK5(state *SessionHandlerStateSDK5) bool {
	...
	*/
	...
	SessionFilterNearRelaysSDK5(state) // IMPORTANT: Reduce % of sessions that run near relay pings for large customers

	return true

}

func SessionFilterNearRelaysSDK5(state *SessionHandlerStateSDK5) {
	...
	*/
	...
	state.Response.DontPingNearRelays = true
}

func SessionMakeRouteDecisionSDK5(state *SessionHandlerStateSDK5) {
		...
		*/
		...
		if stayOnNext {
			...
			if routeChanged {
				...
				BuildContinueTokensSDK5(&state.Output, state.Database, &state.Buyer, &state.Packet, routeNumRelays, routeRelays[:routeNumRelays], state.RouteMatrix.RelayIDs, state.RouterPrivateKey, &state.Response)
			}
...
func SessionPostSDK5(state *SessionHandlerStateSDK5) {
	...
	*/
	...
	if state.Packet.SliceNumber == 0 {
		SessionGetNearRelaysSDK5(state)
	...
	*/
	...
	BuildPostRouteRelayDataSDK5(state)
	...
	*/
	...
	BuildPostNearRelayDataSDK5(state)
	...
	*/
	...
	if state.PostSessionHandler.featureBilling2 && !state.Input.WroteSummary {
		billingEntry2 := BuildBillingEntry2SDK5(state, sliceDuration, nextEnvelopeBytesUp, nextEnvelopeBytesDown, totalPrice)
	...
	*/
	...
	portalData := BuildPortalDataSDK5(state)
...
func BuildPostRouteRelayDataSDK5(state *SessionHandlerStateSDK5) {
...
func BuildPostNearRelayDataSDK5(state *SessionHandlerStateSDK5) {
...
func BuildBillingEntry2SDK5(state *SessionHandlerStateSDK5, sliceDuration uint64, nextEnvelopeBytesUp uint64, nextEnvelopeBytesDown uint64, totalPrice routing.Nibblin) *billing.BillingEntry2 {
	/*
...
func BuildPortalDataSDK5(state *SessionHandlerStateSDK5) *SessionPortalData {
...
func WriteSessionResponseSDK5(
	w io.Writer,
	response *SessionResponsePacketSDK5,
	sessionData *SessionDataSDK5,
	magic []byte,
	backendLoadBalancerIP *net.UDPAddr,
	toAddress *net.UDPAddr,
	backendPrivateKey []byte,
	metrics *metrics.SessionUpdate5Metrics,
) error {
	sessionDataBuffer, err := MarshalSessionDataSDK5(sessionData)
	if err != nil {
		return err
	}
	...
	responsePacketData, err := MarshalPacketSDK5(PacketTypeSessionResponseSDK5, response, magic, backendLoadBalancerIP, toAddress, backendPrivateKey)
	if err != nil {
		return err
	}
...

	{
		...
		fromAddressData, fromAddressPort := core.GetAddressData(backendLoadBalancerIP, fromAddressBuffer[:])
		toAddressData, toAddressPort := core.GetAddressData(toAddress, toAddressBuffer[:])

		if !core.AdvancedPacketFilter(responsePacketData, magic, fromAddressData, fromAddressPort, toAddressData, toAddressPort, len(responsePacketData)) {
			panic("advanced packet filter failed on session response\n")
		}
	}

	if sessionData.RouteState.Next {
		metrics.NextSessionResponsePacketSize.Set(float64(len(responsePacketData) + UDPIPPacketHeaderSize))
	} else {
		metrics.DirectSessionResponsePacketSize.Set(float64(len(responsePacketData) + UDPIPPacketHeaderSize))
	}
...
func SessionUpdateHandlerSDK5Func(
	...
	return func(w io.Writer, incoming *UDPPacket) {
		...
		*/
		...
		state.BackendLoadBalancerIP = backendLoadBalancerIP
		state.PacketFrom = &incoming.From
		...
		state.CurrentMagic = currentMagic[:]
		state.BackendPrivateKey = backendPrivateKey
		state.RouterPrivateKey = routerPrivateKey
		state.Response = SessionResponsePacketSDK5{
		...
		*/
		...
		if state.Packet.SliceNumber == 0 {
			SessionUpdateNewSessionSDK5(&state)
		} else {
			SessionUpdateExistingSessionSDK5(&state)
		}
		...
		*/
		...
		if SessionHandleFallbackToDirectSDK5(&state) {
			return
		}
		...
		*/
		...
		SessionUpdateNearRelayStatsSDK5(&state)

		/*
		   Decide whether we should take network next or not.
		*/

		SessionMakeRouteDecisionSDK5(&state)
...
func MatchDataHandlerSDK5Func(
	getDatabase func() *routing.DatabaseBinWrapper,
	getMagicValues func() ([8]byte, [8]byte, [8]byte),
	PostSessionHandler *PostSessionHandler,
	backendLoadBalancerIP *net.UDPAddr,
	backendPrivateKey []byte,
	metrics *metrics.MatchDataHandler5Metrics,
) UDPHandlerFunc {
	...
	return func(w io.Writer, incoming *UDPPacket) {
		...
		defer func() {
			if err := writeMatchDataResponseSDK5(w, &packet, uint32(responseType), getMagicValues, backendLoadBalancerIP, &incoming.From, backendPrivateKey); err != nil {
