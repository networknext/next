...
func (l *LookerClient) RunSessionMetaDataQuery(sessionID int64, date string, customerCode string, analysisOnly bool) ([]LookerSessionMeta, error) {
	...
	if customerCode != "" {
		if filterExpression != "" {
			filterExpression = fmt.Sprintf("(%s) AND ", filterExpression)
		}

		filterExpression = fmt.Sprintf(`%s(${datacenter_info_v3.customer_code} = "%s" OR is_null(${datacenter_info_v3.customer_code}))`, filterExpression, customerCode)
	}
...
func (l *LookerClient) RunUserSessionsLookupQuery(userID string, userIDHex string, userIDHash string, timeFrame string, customerCode string) ([]LookerUserSessionMeta, error) { // Timeframes 7, 10, 30, 60, 90
	...
	if customerCode != "" {
		filterExpression = fmt.Sprintf(`(%s) AND (${datacenter_info_v3.customer_code} = "%s" OR is_null(${datacenter_info_v3.customer_code}))`, filterExpression, customerCode)
	}
